<script lang="ts">
	import { login, signInWithOAuth } from '$lib/auth/auth';
	import { onMount } from 'svelte';
	import { goto } from '$app/navigation';
	import { isLoggedIn } from '$lib/auth/auth';

	let tab: 'oauth' | 'password' = $state('oauth');
	let identifier = '';
	let password = '';
	let oauthHandle = '';
	let errorMessage = '';
	...
	async function handlePasswordLogin(): Promise<void> {
		if (!identifier.trim() || !password.trim()) {
			errorMessage = 'Please enter your handle and app password.';
			return;
		}
				...
				{#if tab === 'oauth'}
					<div class="space-y-6">
						<p class="text-sm text-gray-400">
							Sign in securely through your PDS — no password shared with this app.
						</p>
						...
						<div>
							...
							<p class="mt-2 text-xs text-gray-400">
								You'll be redirected to your PDS to approve access (only for writing game scores).
							</p>
						</div>

						{#if errorMessage}
							<div class="rounded-xl border border-red-700/50 bg-red-900/50 p-4">
						...
						<button
							...
							class="flex w-full items-center justify-center space-x-2 rounded-xl bg-gradient-to-r from-green-600 to-green-700 px-6 py-4 font-bold text-white shadow-lg transition-all duration-200 hover:from-green-700 hover:to-green-800 disabled:cursor-not-allowed disabled:from-gray-600 disabled:to-gray-700"
						>
							{#if isLoading}
				...
				{:else}
					<form on:submit|preventDefault={handlePasswordLogin} class="space-y-6">
						...
						<div>
							...
							<p class="mt-2 text-xs text-gray-400">
								Your AT Protocol handle or DID. We'll automatically find your server!
							</p>
						</div>
				...
				<details class="group text-sm text-gray-400">
					...
					>
						<div class="flex items-start space-x-3">
							...
							<div class="text-left">
								<p class="font-medium text-gray-300">Granular Permissions</p>
								<p class="text-xs text-gray-400">
									OAuth only requests permission to write game scores — nothing else.
								</p>
							</div>
						</div>
						<div class="flex items-start space-x-3">
							...
							<div class="text-left">
								<p class="font-medium text-gray-300">Secure Authentication</p>
								<p class="text-xs text-gray-400">
									OAuth keeps your password private. App passwords are an alternative if OAuth isn't
									available.
								</p>
							</div>
						</div>
